...
- [x] **5 Ordinals** als iframe+Link mit **invertiertem Farbschema**.
...
- **2026-08-07** — **YouTube fertig.** `scripts/fix-embeds.mjs` wandelt die `ayg-player`-Custom-Elemente in **Datenschutz-Klick-Facades** (`public/js/yt-facade.js`, youtube-nocookie erst bei Klick, Facade-CSS im BaseLayout) → Home-Video + Video-Hauptplayer funktionieren. **Videos-Playlist** (25 Videos, per Playwright aus der Live-Slick-Galerie extrahiert → `src/data/videos-playlist.json`) als responsives **Facade-Grid** (`VideosGallery.astro`, eigene `videos.astro`/`en/videos.astro`). **Hinweis:** Original war ein Slick-Slider → als Grid gebaut (Rückfrage an User offen).
- **2026-08-07** — **Feinschliff-Runde.** (1) **Self-contained CSS:** die letzten externen Assets lokalisiert — **Astra-Icon-Font** (Menü/Such-Icons) + 2 ayg-Spinner nach `public/fonts/astra/` bzw. `public/media/` kopiert, `wp-design.css` umgeschrieben → **0 externe `haushoppe.de`-URLs** (wichtig, da Live jetzt down). (2) **2 BMP-Bilder gefixt** (sips BMP→PNG→webp) → **645/645 Kunstwerke mit Bild**. (3) **Nav-Aktiv-Zustand + Sprachumschalter server-seitig** (`src/lib/nav.ts` + `processChrome()` in `BaseLayout`): jede Seite markiert den korrekten Menüpunkt (Detailseiten heben die Galerie hervor), Switcher zeigt aufs exakte Sprach-Pendant (`src/data/lang-alt.json`, 658 Einträge, Portfolio via `trid`). Kein Client-JS, kein Flackern; im gebauten HTML für alle Seitentypen DE+EN verifiziert. WPML-Flaggen kopiert. (4) **5 Ordinals fertig:** `src/lib/ordinals.ts` + gemeinsame `ArtworkBody.astro` — Detailseite rendert die **Live-Inschrift** (`explorer.ordinalsbot.com`-iframe) + **Gamma-Kauflink** auf **invertiertem schwarzem Design**, statt des statischen Bildes (Galerie-Grid behält den Thumbnail). Playwright-Screenshot GRINDING: iframe lädt die echte Blockchain-Kunst, „Werke" aktiv, Design geflippt — wie beschrieben. `npm run build`: **655 Seiten grün**.
- **KORREKTUR — haushoppe.de ist NICHT offline.** Frühere „offline"-Annahme beruhte auf einem einzelnen `curl`-`000` auf `/nft-kaufen/` (transienter Aussetzer). Verifiziert: Startseite + alle Pfade liefern **HTTP 200** über Cloudflare. **Live steht als exakte Referenz zur Verfügung** — ab jetzt bei jedem Punkt gegen live vergleichen (User-Vorgabe).
- **NFT-Infoseiten: verworfen.** User-Entscheidung: „die alten NFT-Seiten können auch einfach weg" (NFTs sind tot). Nicht geroutet; 2 tote Switcher-Einträge aus `lang-alt.json` entfernt (654 Einträge).
- **2026-08-07** — **Videos-Seite EXAKT wie Original (statt Grid).** User: „exakt so wie vorher". Aus dem **server-gerenderten Live-HTML** den kompletten `ayg`-Classic-Slider-Block verbatim übernommen (`src/data/live-videos{,-en}.json`, je 25 Slides) + die **echten Plugin-Assets** strukturerhaltend gespiegelt (`public/ayg-plugin/…`: jQuery→Slick→ayg-Core→theme-slider) + `ayg_config` lokalisiert injiziert → die Custom Elements `<ayg-player>`/`<ayg-theme-slider>` hydrieren 1:1 (großer Player oben + Slick-Karussell, Prev/Next-Pfeile, 23 Dots, „Now Playing"). Body-Klasse auf **`ast-plain-container`** korrigiert (war Home-`ast-page-builder-template` → Content brach auf 1280 aus). Playwright-Messung lokal vs. live: entry/gallery/player/slider **exakt x=40/w=1200**, DE+EN. EN-WPML-Flaggenpfad gefixt.
- **2026-08-07** — **Portfolio-Detailseiten: Body-Klasse korrigiert** (`src/lib/layout.ts`). Live sind sie **`ast-separate-container ast-two-container`** (boxed, weiße Box, ~48px Padding), ich rendere sie bisher fälschlich full-width. Nach Fix Messung lokal == live: Artikel x=40/w=1200, Titel x=88/w=1104. Bild: meins 1400px (höher aufgelöst als live's 1024er „large"), scharf auf Content-Breite skaliert — gewollt (Rohdaten-Erhalt). Gilt für 322 DE + 322 EN Detailseiten. **Alle Seitentypen haben jetzt die live-korrekte Body-Klasse.**
- **2026-08-07** — **Suche (Pagefind).** Clientseitige Volltextsuche: `pagefind --site dist` im Build-Script erzeugt den Index; `src/components/Search.astro` öffnet ein Overlay beim Klick aufs originale Astra-Such-Icon (lazy geladen, lokalisierte UI). `data-pagefind-body` auf `<main>`, Galerie-Grids via `data-pagefind-ignore` ausgenommen (Detailseiten sind die Suchziele). **DE/EN automatisch getrennt** (über `<html lang>`) — verifiziert: DE-Seite → deutsche UI + DE-Links, EN → englische UI + `/en/`-Links. Index: 2 Sprachen, 651 Seiten.
- **2026-08-07** — **Systematischer Vergleich live↔neu** (Harness: 96 Screenshots, DE+EN, Desktop/Tablet/Mobil, Pixel-Diff-Ranking; Report-Artefakt). Keine 404s, alle 16 Seiten laden. Zwei echte Funde → gefixt:
  - **Kunst Erwerben / Buy Fine Art NEU GEBAUT.** War kaputt (Stackable/UAGB: 2-Spalten stapelte, Akkordeon leer). Statt Gutenberg-CSS zu portieren (User: „hasse Gutenberg, editierbar + einfacher"): **sauberer Nachbau** — `AcquireArt.astro` (2-Spalten-Grid + natives `<details>`-Akkordeon, **kein JS**), Inhalt editierbar in `kunst-erwerben.astro`/`en/buy-fine-art.astro`, Bilder lokalisiert, Live-Tippfehler korrigiert. Aus `[page].astro`/`en/[page].astro` ausgeschlossen. Verifiziert: 2-spaltig, Akkordeon öffnet, Bilder laden, DE+EN.
  - **Galerie-Bildgröße angeglichen.** Mein Justified-Algorithmus packte 4/Reihe (kleiner), live 3/Reihe (größer). Ursache: Zeilenumbruch committete zu früh. Auf **Flickr „closest-to-target"** umgestellt (`public/js/vp-justified.js`) → jetzt große Bilder wie live; Filter neu-layoutet korrekt. Exakte Reihenfolge weicht ab (live sortiert anders) — Bildgröße/Dichte passt.
- **2026-08-07** — **Galerie-Filter gefixt** (User-Fund mobil): war linksbündig/full-width + nach Anzahl sortiert; live ist **zentriert** (`justify-content:center`) + **alphabetisch**. In `Gallery.astro` zentriert/umbrechend gemacht, `galleryCategories` alphabetisch sortiert (`localeCompare`). Verifiziert: Buttons zentriert bei x=344 (identischer Rand links/rechts wie live), Reihenfolge Alle · Digitale Kunst · Gemälde · Holzschnitte · Zeichnungen, mobil 2 Reihen umbrechend — deckungsgleich mit User-Screenshot.
- **2026-08-07** — **Vita + Kontakt sauber neu gebaut** (User: „ja, neu bauen"). Inhalt per Playwright aus live extrahiert → saubere Daten `src/data/vita.json`/`kontakt.json` (Tags balanciert, Tippfehler „öffendliche"→„öffentliche" korrigiert), gerendert über neue **`ContentSections.astro`** (semantische `h2`/`ul`/`p`, zentrierbare Blöcke). Vita: Hero + Lebenslauf (9, mit ausgerichteter Jahres-Spalte) + Ausstellungsverzeichnis (84 + Legende + Link) + Vertretende Galerie + Sammlungen; Kontakt: zentrierte Öffnungszeiten + Adresse (mailto) + Anfahrt. DE+EN verifiziert, aus `[page].astro` ausgeschlossen. **→ Alle Content-Seiten sind jetzt Gutenberg-frei & editierbar** (`live-pages.json`/`[page].astro` damit ungenutzt, können später weg).
- **2026-08-07** — **Kontakt: Karte + Footer-Socials.** Google-Maps-Embed (das beim Screenshot-Vergleich fehlte, weil es per JS lädt) 2-spaltig ergänzt; Facebook + Twitter aus Footer entfernt (DE+EN).
- **2026-08-07** — **GROSSER UMBAU „clean + schlank"** (User: „mach mir das alles hübsch UND schlank"), verifiziert per Playwright gegen live:
  - **Saubere Header/Footer-Komponenten** (`Header.astro`/`Footer.astro` + `site.ts` Daten + `global.css` Design-Tokens exakt aus live gemessen) statt portiertem Astra-Markup. Nav-Leiste pixelgenau (x=370/w=700), Footer `#3a3a3a`. `ast-*`/Seite **39→6**.
  - **Startseite DE+EN neu** (`Home.astro`): Hero-Video-Facade + 2-Spalten, **Gutenberg 18→0**. EN featured = eigenes Werk (Aus dem Zentrum 2023).
  - **CSS geschlankt: `wp-design.css` 557→61 KB (−89 %, PurgeCSS)**; Detail/Galerie pixel-identisch zu live nachgemessen.
  - **Tote Dateien gelöscht:** `live-chrome/home/pages*.json`, `[page].astro` (DE+EN), `lib/nav.ts`.
  - **Rest:** 6–8 `ast-*`/Seite = nur Body-Klasse + `ast-container`-Wrapper (unsichtbares Breiten-Gerüst); optionaler letzter Schritt.
- **Stand:** Migration + Clean-up komplett, alle Seitentypen DE+EN gegen live geprüft, keine offenen Fehler, 0 Gutenberg, CSS schlank. **Nächster Schritt: deine finale lokale Abnahme → Cloudflare-Deploy** (zwei Domains, braucht deine Secrets). Deploy-CSP: `img-src i.ytimg.com`, `frame-src youtube-nocookie.com explorer.ordinalsbot.com www.google.com`, Pagefind/ayg laden nur eigene Assets.

...
- **BLOCKER — NFT-Infoseiten:** Inhalt liegt vollständig vor (nft-kaufen/nft-buy je ~86k, hintergrundwissen/background-knowledge je ~8,7k, oncyber ~43k Zeichen), **aber** die Seiten sind reine **Stackable/UAGB-Blöcke**, deren seitenspezifisches CSS (`uniqueClass` z. B. `ugb-fcd1a11`) zur Render-Zeit inline im `<head>` erzeugt wurde — **lokal nirgends vorhanden** (0/31 in `custom-style-blocks.css`/autoptimize). **haushoppe.de ist offline** (kein Live-HTML mehr), **Wayback rate-limitet** (429). Ohne gerenderte Quelle kein design-exakter Port möglich → **Entscheidung nötig** (siehe Rückfragen).
- **Offene Rückfragen an User:** (a) NFT-Seiten: Wayback-Rekonstruktion versuchen · vs. vereinfachtes Eigen-Design · vs. weglassen? (b) Videos: aktuelles **Grid** ok oder original **Slick-Slider** exakt? — Danach nur noch Cloudflare-Deploy (braucht deine Secrets).

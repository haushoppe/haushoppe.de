...
- **Nächste Schritte (Loop):** **EN-Zweig** (haushoppe.art → `/en/…`, gleiche Bausteine für die 322 EN-Werke + EN-Seiten), **YouTube-Facade** (Home + Videos-Seite, Playlist), **5 Ordinals** (iframe+Link, invertierte Farben), NFT-Infoseiten (nur fertige), Nav-Aktiv-Zustand + Sprachumschalter dynamisch, 2 BMP-Bilder fixen, `//haushoppe.de`-Plugin-Assets in der CSS lokalisieren. Danach vollständige lokale Abnahme → dann erst Cloudflare-Deploy (braucht deine Secrets = Blocker).

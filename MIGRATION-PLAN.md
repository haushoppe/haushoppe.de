...
- **Offen:** vollständige lokale Abnahme durch User → dann Cloudflare-Deploy (zwei Domains, braucht deine Secrets). Hinweis Deploy: Videos-Seite lädt YouTube-Thumbnails von `i.ytimg.com` + der Player YouTube erst bei Klick — CSP entsprechend setzen (`frame-src youtube`, `img-src i.ytimg.com`, `frame-src explorer.ordinalsbot.com` für Ordinals).
